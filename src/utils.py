...
def extract_audio_from_video(
    video_file_path: str | Path, output_audio_file_path: str | Path
):
...
def adjust_noise_levels(
    redubbed_audio: AudioSegment, audio_target: AudioSegment
) -> AudioSegment:
    """
    Adjust noise level of redubbed audio to original audio noise level
    """
    noise_part = to_array_fp32(
        audio_target
    )  # Assume first 1s contains background noise
    redubbed_audio_noise = nr.reduce_noise(
        y=to_array_fp32(redubbed_audio), sr=audio_target.frame_rate, y_noise=noise_part
    )
    return AudioSegment(
    ...
    for redubbed_segment in redubbed_audios:
        ...
        if speed_factor > 0.85:
            ...
            silence = AudioSegment.silent(
                duration=(required_duration - len(redubbed_audio)) // 2,
                frame_rate=redubbed_audio.frame_rate,
            )
            redubbed_audio = silence + redubbed_audio + silence
            redubbed_audio = adjust_noise_levels(
                redubbed_audio, original_audio[start_time * 1000 : end_time * 1000]
            )
        ...
        original_audio = (
            original_audio[: start_time * 1000].fade_out(fade_level)
            + redubbed_audio.fade_in(fade_level).fade_out(fade_level)
            + original_audio[end_time * 1000 :].fade_in(fade_level)
        )
